        ...
        file.add(item("Run current query", x->{
            ...
            if (query_file != null) {
                ...
                try (FileInputStream fis = new FileInputStream("configuration.properties")) {
                    prop.load(fis);
                    var output_folder = Paths.get("visualizer","python", "data").toFile();
                    ...
                    String[] commands = new String[]{Path.of(cpp).toString(),
                    ...
                    Files.copy(data.toPath(), Path.of(output_folder.getAbsolutePath(), "sentences.txt"), REPLACE_EXISTING);
