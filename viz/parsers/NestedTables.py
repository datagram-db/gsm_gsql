...
class Table:
    ...
    def body(self):
        return "<tbody>"+("".join(map(lambda x: "<tr onclick=\"callrow(this)\">"+("".join(map(lambda y: "<td>"+toSerialize(y)+"</td>", zip(x,self.Schema))))+"</tr>", self.rows)))+"</tbody>"
...
def parse_cell(s:str, G):
    ...
    if s.startswith("@"):
        return parse_nested_table(s[1:], G)
...
def parse_nested_table(s:str, G):
    G = int(G)
    schema = list()
    # table = list()
    # s = skipFirst(s, ',')
...
def generate_morphism_html(path, name):
    ...
    d = dict()
#     strR = """
#         <html><head><title>Morphisms</title>
#         <!-- Import Pure CSS -->
#         <style type="text/css"
#
#         >
...
#     }
#
#                function generateFromNode(nodeObject,nodeID)  {
...
#             var request = "http://127.0.0.1:9999/input/node/§/"+$(this).text();
#             $.getJSON( request, function( dataNodes ) {
#                 generateFromNode(dataNodes,$(this).text());
#                 $("div.tooltip").css({
...
#         </head><body onload="draw()"><div class="tooltip">
#     <span class="close">X</span>
#     <span id="contento"></span>
# </div>
#         """
#     strR = strR.replace("§", name)
    ...
    strR = ""
    for x in d:
        d[x] = "".join(map(lambda x: "<p>" + str(x[1]) + "</p>", sorted(d[x].items(), key=lambda x: int(x[0]))))
    for header in sorted(d.keys()):
        strR = strR + "<h1>" + header + "</h1>" + d[header]
    # element = """
    #
    # """
    # strR = strR + "</body></html>"
