...
//                    raw_table.score = dstE+","+var_table_edge;//"IEs("+dstE+","+raw_table.edge+")";
                ...
                if (isForall && isDstForall) {
                    table = nest_or_groupby(table, first, nesting);
...
//                    std::cout << "BENE" << std::endl;
    ...
    void instantiate_relational_tables(const std::vector<node_match>& vl) {
...
//                        auto& ref = M[std::make_pair(std::get<size_t>(rec.at(0).val), std::get<size_t>(rec.at(1).val))];
                ...
                result = nest_or_groupby(result, all, "*");
                idx_to_remove.clear();
...
//                    }
    ...
    void collect_node_match(node_match& ref,
...
//        }
    ...
    void finalise_after_all_collections() {
...
int main() {
    ...
    gsm2::tables::primary_memory_load_gsm2( "./data/einstein.txt", forloading);
    ...
    pr.finalise_after_all_collections();
    pr.run_simple_edge_queries(forloading);
    pr.instantiate_relational_tables(vl);

    for (const auto& g : forloading.all_indices) { // In parallel do!

        for (const auto& [time, vertices] : g.container_order) {
    ...
    pr.print_preliminary_edge_match_results(std::cout);
//    std::cout << "<" << std::endl;
//
//
